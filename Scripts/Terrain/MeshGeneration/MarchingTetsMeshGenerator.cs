...
namespace Evix.Terrain.MeshGeneration {
  ...
  public static class MarchingTetsMeshGenerator {
    ...
    static int GetVoxelsToMarchOver(Coordinate chunkID, Level level, out byte[] voxels) {
      ...
      chunkWorldLocation.until(chunkWorldLocation + MarchDiameter, (worldLocation) => {
        ...
        if (voxel != Evix.Voxels.Voxel.Types.Empty.Id) {
          collectedVoxels[(worldLocation - chunkWorldLocation).flatten(MarchDiameter)] = voxel;
          solidVoxelCount++;
        }
      });

      voxels = collectedVoxels;
    ...
    /// <param name="chunkID"></param>
    /// <param name="level"></param>
    public static void ForEachRequiredNeighbor(Coordinate chunkID, Level level, Func<Chunk, bool> action) {
      foreach (Coordinate neighborOffset in NecessaryLoadedChunkNeighborOffsets) {
        Coordinate neighborKey = chunkID + neighborOffset;
        Chunk neighbor = level.getChunk(neighborKey);
        if (!action(neighbor)) {
    ...
    static Color UVInterpolate(IVoxelType voxelTypeOne, IVoxelType voxelTypeTwo) {
      ...
      return voxelTypeOne.Color.LinearInterpolate(voxelTypeTwo.Color, 0.5f);
    }
    ...
    /// <param name="level"></param>
    /// <returns></returns>
    public static MarchingTetsMeshGenJob GetJob(ChunkResolutionAperture.Adjustment adjustment, Level level) {
      return new MarchingTetsMeshGenJob(adjustment, level);
    }

    /// <summary>
    /// A unity parallel job to run to generate the marched mesh values.
    /// </summary>
    public struct MarchingTetsMeshGenJob : ChunkResolutionAperture.IAdjustmentJob {

      /// <summary>
      /// The adjustment we're working on
      /// </summary>
      ...
      internal MarchingTetsMeshGenJob(ChunkResolutionAperture.Adjustment adjustment, Level level) {
      ...
      public void doWork() {
        ...
        if (solidVoxelCount > 0) {
          ...
          if (generatedVerticies.Count > 0) {
            ...
            surfaceArray[(int)ArrayMesh.ArrayType.Index] = generatedTriangles.ToArray();
            surfaceArray[(int)ArrayMesh.ArrayType.Color] = generatedColors.ToArray();
